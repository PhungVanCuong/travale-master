    ...
    <form action="{{ route('cancel-booking') }}" method="POST" class="booking-container">
        ...
        <div class="booking-info">
            ...
            <div class="booking__infor">
                <div class="form-group">
                    ...
                    <input type="text" id="username" placeholder="Nhập Họ và tên" name="fullName"
                        value="{{ $tour_booked->user->username ?? '' }}" readonly>
                ...
                <div class="form-group">
                    ...
                    <input type="email" id="email" placeholder="sample@gmail.com" name="email"
                        value="{{ $tour_booked->user->email ?? '' }}" readonly>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="tel" placeholder="Nhập số điện thoại liên hệ" name="tel"
                        value="{{ $tour_booked->user->phoneNumber ?? '' }}" readonly>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="address" placeholder="Nhập địa chỉ liên hệ" name="address"
                        value="{{ $tour_booked->user->address ?? '' }}" readonly>
            ...
            <label class="payment-option">
                <input type="radio" value="office-payment" @if (isset($tour_booked->checkout) && $tour_booked->checkout->paymentMethod == 'office-payment') checked @endif disabled>
                <img src="{{ asset('clients/assets/images/contact/icon.png') }}" alt="Office Payment">
                Thanh toán tại văn phòng
            </label>

            <label class="payment-option">
                <input type="radio" value="paypal-payment" @if (isset($tour_booked->checkout) && $tour_booked->checkout->paymentMethod == 'paypal-payment') checked @endif disabled>
                <img src="{{ asset('clients/assets/images/booking/cong-thanh-toan-paypal.jpg') }}" alt="PayPal">
                Thanh toán bằng PayPal
            </label>

            <label class="payment-option">
                <input type="radio" value="momo-payment" @if (isset($tour_booked->checkout) && $tour_booked->checkout->paymentMethod == 'MOMO') checked @endif disabled>
        ...
        <div class="booking-summary">
            <div class="summary-section">
                <div>
                    ...
                    <h5 class="widget-title">{{ $tour_booked->tour->title ?? '' }}</h5>
                    <p>Ngày khởi hành : {{ isset($tour_booked->tour) ? date('d-m-Y', strtotime($tour_booked->tour->startDate)) : '' }}</p>
                    <p>Ngày kết thúc : {{ isset($tour_booked->tour) ? date('d-m-Y', strtotime($tour_booked->tour->endDate)) : '' }}</p>
                </div>
                ...
                <div class="order-summary" style="border-bottom: 1px solid #d6d6d6; margin-bottom:20px">
                    <div class="summary-item">
                        ...
                        <div>
                            ...
                            <span>X</span>
                            <span class="total-price-booked">{{ number_format($tour_booked->tour->priceAdult ?? 0, 0, ',', '.') }}
                                VNĐ</span>
                        </div>
                    </div>
                    <div class="summary-item">
                        ...
                        <div>
                            ...
                            <span>X</span>
                            <span class="total-price-booked">{{ number_format($tour_booked->tour->priceChild ?? 0, 0, ',', '.') }}
                                VNĐ</span>
                        </div>
                    </div>
                    <div class="summary-item">
                        ...
                        <div>
                            <span class="total-price-booked">
                                @if(isset($tour_booked->tour))
                                    {{ number_format(($tour_booked->numAdults * $tour_booked->tour->priceAdult + $tour_booked->numChildren * $tour_booked->tour->priceChild) - $tour_booked->totalPrice, 0, ',', '.') }}
                                @else
                                    0
                                @endif
                                VNĐ
