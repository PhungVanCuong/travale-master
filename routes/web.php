...
Route::get('activate-account/{token}', [LoginController::class, 'activate'])->name('activate'); // Đã sửa name và function khớp Controller
...
Route::get('/booking', [BookingController::class, 'handlePaymentMomoCallback'])->name('momo.callback'); // Đã đổi route name thành momo.callback
...
// Search
Route::get('/search', [SearchController::class, 'index'])->name('search'); // Đã bỏ name: dư thừa
...
Route::prefix('admin')->middleware('admin')->group(function () {
    ...
    Route::post('/edit-tour', [ToursManagementController::class, 'editTour'])->name('admin.edit-tour'); // Đã sửa tên function

    // (Bỏ comment route này nếu hệ thống có thêm ảnh temp, nhưng logic dropzone hiện tại gọi qua add-images-tours)
    ...
    Route::get('/booking-detail/{id?}', [BookingManagementController::class, 'bookingDetail'])->name('admin.booking-detail'); // Đã sửa tên function
